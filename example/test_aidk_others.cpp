...
int main(int argc, char** argv) {
    ...
    // set variable
    if (version >= "v3.1.0"){
        std::unordered_map<std::string, flexiv::aidk::value_variant> vars = client.get_direct_setting_variables();
        ...
        std::unordered_map<std::string, flexiv::aidk::value_variant> set_vars;
    ...
    if (version >= "v2.11.1"){
        ...
        if(!status) std::cerr<<"save config failed!"<<std::endl;

        status = client.reload_configs(js["project"]);
        if(!status) std::cerr<<"reload config failed for project: "<< js["project"] <<std::endl;
        std::cout<<"reload project: " << js["project"] << " state: " << status << std::endl;

        status = client.warmup();
        if(!status) std::cerr<<"warmup failed!"<<std::endl;
    }
    ...
    if (version >= "v3.1.0"){
        ...
        // file transfer
        client.send_file("../../include/aidk.hpp", "model/test.hpp");
